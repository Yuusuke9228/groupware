...
- 設備予約を日・週・月のカレンダー表示に対応しました。
- 機能ごとの利用可否と作成許可ロールを管理者設定に追加しました。
- ホームに社内システム入口としてポータルリンクを追加しました。
- WEBデータベースの定期CSVインポート/エクスポートを自動化ジョブに追加しました。
- 正式版 v1.0.0 としてリリースしました。
