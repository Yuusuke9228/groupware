...
<div class="container-fluid mt-4">
    <div class="d-flex flex-wrap justify-content-between align-items-center gap-2 mb-3">
        <div>
            <h4 class="mb-1"><i class="fas fa-cloud me-2 text-primary"></i><?= htmlspecialchars(tr_text('Drive', 'Drive')) ?></h4>
            <div class="text-muted small"><?= htmlspecialchars(tr_text('通常のファイル管理とは独立した共有ストレージです。', 'Independent shared storage separated from File Manager.')) ?></div>
        </div>
        <div class="d-flex gap-2">
            <a href="<?= BASE_PATH ?>/drive/upload" class="btn btn-primary">
    ...
    <div class="card mb-3">
        <div class="card-body">
            <form method="get" action="<?= BASE_PATH ?>/drive" class="row g-2">
                <div class="col-md-10">
        ...
        <div class="card-body p-0">
            <?php if (empty($items)): ?>
                <div class="text-center text-muted py-5">
                    ...
                    <div><?= htmlspecialchars(tr_text('Driveにファイルはありません。', 'No files in Drive yet.')) ?></div>
                </div>
            <?php else: ?>
                    ...
                    <table class="table table-hover mb-0 align-middle">
                        <thead class="table-light">
                            <tr>
                                ...
                                <th style="width: 220px;"><?= htmlspecialchars(tr_text('操作', 'Actions')) ?></th>
                            </tr>
                        </thead>
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td>
                                    <div class="d-flex gap-1 flex-wrap">
                                        <a href="<?= BASE_PATH ?>/drive/file/<?= (int)$item['id'] ?>" class="btn btn-sm btn-outline-secondary">
                                            <i class="fas fa-eye me-1"></i><?= htmlspecialchars(tr_text('詳細', 'Details')) ?>
                                        </a>
                                        <a href="<?= BASE_PATH ?>/drive/download/<?= (int)$item['id'] ?>" class="btn btn-sm btn-outline-primary">
