...
$availableOrganizations = $availableOrganizations ?? ($organizations ?? []);
$facilityOptions = $facilityOptions ?? [];
$selectedParticipants = $participants ?? ($schedule['participants'] ?? []);
$selectedOrganizations = $sharedOrganizations ?? ($schedule['organizations'] ?? []);
$selectedFacilityIds = $selectedFacilityIds ?? ($schedule['facility_ids'] ?? []);

$selectedParticipantIds = array_map(function ($participant) {
    if (is_array($participant)) {
        return (int)($participant['id'] ?? 0);
    }
    return (int)$participant;
}, is_array($selectedParticipants) ? $selectedParticipants : []);

$selectedOrganizationIds = array_map(function ($organization) {
    if (is_array($organization)) {
        return (int)($organization['id'] ?? 0);
    }
    return (int)$organization;
}, is_array($selectedOrganizations) ? $selectedOrganizations : []);

$selectedFacilityIds = array_map(function ($facilityId) {
    return (int)$facilityId;
}, is_array($selectedFacilityIds) ? $selectedFacilityIds : []);
$selectedFacilityIds = array_values(array_unique(array_filter($selectedFacilityIds, function ($facilityId) {
    return $facilityId > 0;
})));

// 各フィールドの値取得用ヘルパー
$getValue = function ($field, $default = '') use (&$schedule) {
...
<div class="mb-3">
    ...
    <select class="form-select" id="facility_ids" name="facility_ids[]" multiple>
        <?php foreach ($facilityOptions as $facility): ?>
            ...
            <option value="<?php echo $facilityId; ?>" <?php echo in_array($facilityId, $selectedFacilityIds, true) ? 'selected' : ''; ?>>
...
<div class="mb-3 visibility-specific" style="<?php echo $getValue('visibility') === 'specific' ? '' : 'display: none;'; ?>">
    ...
    <select class="form-select participant-select" id="participants" name="participants[]" multiple>
        <?php if (is_array($availableUsers)): ?>
            <?php foreach ($availableUsers as $userOption): ?>
                <?php $userId = (int)($userOption['id'] ?? 0); ?>
                <option value="<?php echo $userId; ?>" <?php echo in_array($userId, $selectedParticipantIds, true) ? 'selected' : ''; ?>>
                    <?php echo htmlspecialchars(($userOption['display_name'] ?? '') . ' (' . ($userOption['username'] ?? '') . ')'); ?>
                </option>
            <?php endforeach; ?>
        <?php endif; ?>
    </select>
...
<div class="mb-3 visibility-specific" style="<?php echo $getValue('visibility') === 'specific' ? '' : 'display: none;'; ?>">
    ...
    <select class="form-select organization-select" id="organizations" name="organizations[]" multiple>
        <?php if (is_array($availableOrganizations)): ?>
            <?php foreach ($availableOrganizations as $org): ?>
                <?php $orgId = (int)($org['id'] ?? 0); ?>
                <option value="<?php echo $orgId; ?>" <?php echo in_array($orgId, $selectedOrganizationIds, true) ? 'selected' : ''; ?>>
